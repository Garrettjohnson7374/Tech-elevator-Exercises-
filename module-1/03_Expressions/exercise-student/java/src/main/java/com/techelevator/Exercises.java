...
public class Exercises {
	...
	public boolean loneTeen(int a, int b) {
		if (((a >= 13) && (a <= 19)) && ((b >= 13) && (b <= 19))) { // both in range false
			return false;
		} else if ((a >= 13 && a <= 19) && (b <= 13 || b >= 19)) { //a in range b out true
			return true;
		} else if ((b >= 13 && b <= 19) && (a <= 13 || a >= 19)) { //b in range a out true
			return true;
		}
		return false; // both out false
	}
	...
	public int max1020(int a, int b) {
		if ((a > b) && (a >= 10) && (a <= 20)) {        //a in range and larger than b
			return a;
		} else if ((b > a) && (b >= 10) && (b <= 20)) {        //b in range and larger than a
			return b;
		} else if ((a < b) && (b >= 21) && (a >= 10) && (a <= 20)) {    // a is less but in range
			return a;
		} else if ((b < a) && (a >= 21)) {                        // b is less but in range
			return b;
	...
	/*
	 ...
	 result is 1 (maybe).                    if  >=8  always return 2
	 dateFashion(5, 10) → 2					 if  <=2  always return 0
	 dateFashion(5, 2) → 0					 if between 7 and 3 return 1
	 dateFashion(5, 5) → 1
	 */
	public int dateFashion(int you, int date) {
		if ((you <= 2) || (date <= 2))
			...
			return 2;
		} else if ((you <= 7) && (you >= 3) || (date <= 7) && (date >= 3)) ;
		{
			return 1;
		}
	}
	...
	/*
	 ...
	 squirrelPlay(70, false) → true						summer false >=60 and <=90 plays
	...
	/*
     ...
     yourCakeAndEatItToo(4.99, false) → "standard"			if birthday +5
     yourCakeAndEatItToo(4.99, true) → "standard"			<= $10 (standard)
     yourCakeAndEatItToo(7.00, false) → "standard"			<= $15 (special)
     yourCakeAndEatItToo(7.00, true) → "special"			> $15 (ginormous)
	...
	public String yourCakeAndEatItToo(double mealAmount, boolean isBirthday) {
		if (isBirthday == true && mealAmount <= 5) {
			return "standard";
		} else if (isBirthday == true && mealAmount > 5 && mealAmount <= 10) {
			return "special";
		} else if (isBirthday == true && mealAmount >= 10) {
			return "ginormous";
	...
	/*
	 ...
	 alarmClock(1, false) → "7:00"						weekday return "7:00"
	 alarmClock(5, false) → "7:00"				if vacation And weekday 1-5 return "10:00"
	 alarmClock(0, false) → "10:00"				if vacation AND weekend return "off"
	 */                                        //	if weekend return "10:00"
	public String alarmClock(int day, boolean vacation) {
		if (vacation && (day == 6 || day == 0))                                    // weekend vacation
			return "off";

		else if (vacation && day > 0) {                                            //weekday vacation
			return "10:00";
		} else if (vacation == false && (day == 6) || (day == 0))                        // weekend
		{
			return "10:00";
	...
	public boolean in1To10(int n, boolean outsideMode) {
		if (outsideMode && (n <= 1 || n >= 10)) {
			return true;
		} else if ((outsideMode) && (n <= 2) && (n >= 9)) {
			return false;
		} else if ((outsideMode == false) && (n > 0) && (n < 11)) {
			return true;
		} else {
			return false;
		}
